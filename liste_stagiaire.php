...
<div>
	...
	<div id="contenu1"> 
		...
		$recherche=
            "SELECT * FROM
            stagiaires 
            INNER JOIN encadreurs
            ON stagiaires.matricule=encadreurs.matricule 
            INNER JOIN services 
            ON stagiaires.codeserv=services.codeserv order by nom";
        ...
        {
		echo'<tr>';
		echo'<td><a href="modifie_stagiaire.php?modifistag='.$data['numstagiaire'].'" >Modifier</a></td>
        
        
        <td><a href="function_supprimer_stagiaire.php?suppstag='.$data['numstagiaire'].'" onclick="return(confirm(\'Etes-vous sur de vouloir supprimer ce stagiaire?\'));" >Supprimer</a></td>
        
        <td>'.$data['numstagiaire'].'</td><td>'.strtoupper($data['nom']).'</td>
        
        
        <td>'.strtoupper($data['prenom']).'</td><td><img width="60" height="70" src="'.$data['photo'].' "></td><td><a href="" >Voir</a></td>';
		echo'</tr>';
		}
		?>
		</table></center>
